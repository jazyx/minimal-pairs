...
export const PairsProvider = ({ children }) => {
  const { pair } = useContext(PreferencesContext)
  ...
  function setPhonemePair(pair) {
    ...
    if (index < 0) {
      setCurrentPair(phonemePairs[0])
    }
  
    _setPairListAndPhonemeSymbols(pair || phonemePairs[0])
  ...
  function _setPairListAndPhonemeSymbols(pair) {
    setPhonemeSymbols(pairs.index[pair].phonemes)
    // [<ɪ>, <æ>]
